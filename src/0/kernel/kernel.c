...
void kernel_main(unsigned long bootloader_header_magic, unsigned long bootloader_info_addr) 
{
	...
	terminal_writestring("bootloader_info_addr = ");
	...
	if (bootloader_header_magic == MULTIBOOT_BOOTLOADER_MAGIC) {
		...
		(void)itoa((unsigned)info_from_bootloader->flags,(unsigned char*)buffer,lengthOfIntString,16);
		terminal_writestring(buffer);
		terminal_writestring("\n");
		terminal_writestring("RAM lower: ");
		(void)itoa(info_from_bootloader->mem_lower,(unsigned char*)buffer,lengthOfIntString,16);
		terminal_writestring(buffer);
		terminal_writestring("\n");
		//terminal_writestring("RAM upper: %x\n", (unsigned)info_from_bootloader->mem_upper);
//		terminal_writestring("RAM upper: %x\n", (unsigned)info_from_bootloader->mem_upper);
		terminal_writestring("RAM upper: ");
		(void)itoa(info_from_bootloader->mem_upper,(unsigned char*)buffer,lengthOfIntString,16);
		terminal_writestring(buffer);
		terminal_writestring("\n");
