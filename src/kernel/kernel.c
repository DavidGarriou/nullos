...
void kernel_main(unsigned long bootloader_header_magic, unsigned long bootloader_info_addr) 
{
	...
	unsigned long mbi_size, *ptr, *head;
...
//	printf("\n");
	...
	if (bootloader_header_magic == MULTIBOOT2_BOOTLOADER_MAGIC) {
		/* Loaded with Grub */
		...
		printf("bootloader_info_addr = %x\n", bootloader_info_addr);
		head = (unsigned long *) bootloader_info_addr;
		ptr = head;
		printf("ptr     = %x, *ptr = %x\n", ptr, *ptr);
//		printf("*ptr + 1 = %x\n", *(ptr+1));
		ptr = head + 8;
